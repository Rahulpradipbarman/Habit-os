...
const HabitosHeroAccordion = dynamic(() => import('@/components/ui/habitos-hero-accordion').then(mod => mod.HabitosHeroAccordion), { ssr: false });
...
export default function HomePageClient({ initialIsLoggedIn }: { initialIsLoggedIn: boolean }) {
      ...
      <header className="sticky top-0 z-50 bg-white shadow-sm border-b border-outline-variant/10">
        <nav className="relative flex justify-between items-center w-full px-6 py-4 max-w-[1440px] mx-auto">
          ...
          <div className="hidden md:flex items-center justify-center gap-8 absolute left-1/2 -translate-x-1/2">
            ...
            <Link prefetch={true} className="text-xs font-bold text-on-surface-variant hover:text-primary transition-colors" href={isLoggedIn ? "/today?tab=library" : "/login"}>Habit Library</Link>
            <Link prefetch={true} className="text-xs font-bold text-on-surface-variant hover:text-primary transition-colors" href={isLoggedIn ? "/today?tab=community" : "/login"}>Community</Link>
          </div>
            ...
            {isLoggedIn ? (
              <div className="hidden md:flex items-center gap-4">
                <Link 
                  ...
                  className="px-5 py-2 bg-primary text-white text-xs font-bold rounded-full shadow-sm hover:opacity-95 active:scale-95 transition-all flex items-center justify-center"
                >
                  Go to Dashboard
                </Link>
                <button 
        ...
        >
          <div className="flex flex-col px-6 py-4 gap-4">
            ...
            <Link prefetch={true} className="text-sm font-bold text-on-surface-variant hover:text-primary transition-colors" href={isLoggedIn ? "/today?tab=library" : "/login"}>Habit Library</Link>
            <Link prefetch={true} className="text-sm font-bold text-on-surface-variant hover:text-primary transition-colors" href={isLoggedIn ? "/today?tab=community" : "/login"}>Community</Link>
            ...
            {isLoggedIn ? (
              <div className="flex flex-col gap-3">
                <Link 
                  href="/today?tab=dashboard" 
                  className="w-full text-center px-5 py-2.5 bg-primary text-white text-sm font-bold rounded-lg shadow-sm hover:opacity-95 transition-all flex items-center justify-center"
                >
                  Go to Dashboard
                </Link>
                <button 
            ...
            <div className="flex flex-wrap gap-4 pt-2">
              {isLoggedIn ? (
                <Link 
                  prefetch={true}
                  href="/today?tab=dashboard"
                  className="px-8 py-3 bg-primary text-white rounded-full font-bold text-xs hover:scale-105 active:scale-95 transition-all shadow-lg shadow-primary/20 flex items-center justify-center"
                >
                  Go to Dashboard
                </Link>
              ) : (
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            ...
            <div className="md:col-span-12 bg-surface-container-low p-6 rounded-2xl border border-outline-variant/20 flex flex-col md:flex-row items-center gap-6">
              ...
              <div className="md:w-1/2 flex -space-x-3 overflow-hidden py-4 justify-center md:justify-end">
                <Image width={48} height={48} className="inline-block h-12 w-12 rounded-full ring-4 ring-white" alt="Community Member" src="https://lh3.googleusercontent.com/aida-public/AB6AXuCxUHRe0_K-Px1jLed_XfYFr0GW1_6jEQIIaNC_eUqxHW8DNFRrSrUXS0MIqA8jzMGsu4otucgtIxcmU1UmL-ssk_dfJqwVPrxe03lUeBq0ECVcCaLw68mq8pKc2YbFaDD7_N93h6cIZ0XWptrC1CEiYN6U_i6DMd2B38mvGzKbWq9V3i46mAnIjnw1WTFYqtEzfrZbniLNsOeTr--FxVCDSxtaB-yFhFkpp5Jp3Mni8jvzTXuZ8T8J0qHU3_7ZPnrMyocTCqu28m4"/>
                <Image width={48} height={48} className="inline-block h-12 w-12 rounded-full ring-4 ring-white" alt="Community Member" src="https://lh3.googleusercontent.com/aida-public/AB6AXuDLd-R6BYftVTXXai2C1XZmqxEiGyvsjVH_Y71xMFflNEMZPOyv3qwXc38CHcenor0wlJ5xwe7ya4ueo3_L0LDORrubuL_SnaFhN3w9iy5asUap-61nVjZxBRcj6YqNR5icxKgcjj-BVn1JVd0Yog2nUoq1MsKKeoBpQHBFl63Mk5iylqW18_Kxwh_dfvBuUboegPhSbFr4sYh8IO4Tq4lwU2JF-sWV75tVZ7afrPyaI1Dw98d3sZv49EghRLwLbclu0JEAqBFPfH0"/>
                <Image width={48} height={48} className="inline-block h-12 w-12 rounded-full ring-4 ring-white" alt="Community Member" src="https://lh3.googleusercontent.com/aida-public/AB6AXuC2u_Hyv6gWuSMLcYxt8sQ3yyL4GEGYhFWk-nlf-DlolOICmHVYkBPSzcW8rmZ82tdBeg-xdp7vBJDOyB7e6p9RLcnufSVnROOef9txos61quCjF1c2u_BaRZUJlzhZwCt4GmxhXmjgeeqZc67CU6diMNkQoN6faYXuaiOef4i_DQIykK37ukLraAzaFZZk4GKBos2mpSX5FuxqxzgVTQHYBEq3VBEy6dw4LHdRmh7RdKDtVFqrWdKQ7DesxhJ_DZmCC8YB-kG9HIc"/>
                <Image width={48} height={48} className="inline-block h-12 w-12 rounded-full ring-4 ring-white" alt="Community Member" src="https://lh3.googleusercontent.com/aida-public/AB6AXuANipRWd07j_xq77Knh5QaFeHkrGBtPhNtGm5nlb1_O0aVwgFesacezS1wS4IOJhxcyjBCV-82HqgXkepgdUHIzTpmmKR16Ridfma4Bm5aOVRDvchZ0QkmOupMcj4Ki36yuIQxxOIXbKSnwJc4YItfVpWyDJHWTdgRSx8ORpbi598Hc_TDsKYzyEyIzTxd0D7TnYXAhJeAcyDoFyBwbfHnEhViyW9cb2dh0qJ_r8C42uevQ5RHWx0zpflQT1CDPPM3M2Teh5aySQz0"/>
            ...
            <div className="flex flex-col sm:flex-row gap-3 relative z-10 w-full justify-center max-w-xs">
              {isLoggedIn ? (
                <Link 
                  prefetch={true}
                  href="/today?tab=dashboard"
                  className="px-6 py-2.5 bg-white text-primary rounded-full font-bold text-xs hover:scale-105 active:scale-95 transition-all text-center flex items-center justify-center"
                >
                  Go to Dashboard
                </Link>
              ) : (
